...
if not configReq.content then
    gg.alert("❌ Cannot connect to server.")
    os.exit()
end

local ok, config = pcall(function() return load("return " .. configReq.content)() end)
if not ok or not config then
    gg.alert("❌ Invalid config file.")
    os.exit()
end

if config.status ~= "on" then
    gg.alert("🚫 Script is turned OFF by the developer.")
    os.exit()
end
